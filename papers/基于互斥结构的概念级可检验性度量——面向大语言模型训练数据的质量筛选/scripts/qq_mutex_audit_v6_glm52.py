# -*- coding: utf-8 -*-
"""互斥审查器 v6：LLM 消融实验——GLM5.2 跑完整双模实验
版本备案：v6（换 LLM：deepseek-chat → glm-5.2 @ tokenrhythm.studio；提示词与 v5.6 相同）
流程（对齐 v5.6）：
  ① 变体结构提取（主题聚类 + event/prediction/is_anchored/anchor_interface/theme）
  ② 公理 1 锚定判定（检验方案 + 论证链终点 + 三要件 + 自足定义排除）
  ③ 核验层（接口存在性 + 事实结果 → align）
  ④ 确定性判决（μ/A_rate/r/Δr/align + 置信度）
输出：mutex_audit_v6_glm52.json（对照数据，不覆盖 v5 系列）
目的：换 LLM 后判决稳定性（消融实验第一轮）
"""
...
def llm(prompt, model=MODEL, max_tokens=3000, temperature=0.1, retries=15):
    for attempt in range(retries):
        try:
            body = {"model": model, "messages": [{"role": "user", "content": prompt}],
                    "max_tokens": max_tokens, "temperature": temperature}
            resp = requests.post(API_URL, json=body,
                headers={"Authorization": "Bearer" + " " + API_KEY}, timeout=(20, 300))
            if resp.status_code == 503:
                wait = 10 * (attempt + 1)
                print(f"  [503] wait {wait}s (attempt {attempt+1})", flush=True)
                time.sleep(wait)
                continue
...
# ---------- ① 变体结构提取（v5.1 prompt，主题聚类版） ----------
def extract_variant_struct(concept_name, variants):
    slist = []
    for i, v in enumerate(variants):
        label = v.get("label", "?")
        stance = (v.get("stance") or "")[:100]
        pol = v.get("polarity", v.get("type", "?"))
        src = v.get("source", "")
        slist.append(f"[{i+1}] label={label} | polarity={pol} | source={src} | stance={stance}")
    sblock = "\n".join(slist)
...
def decide_mu(card, mu_star_lo=0.037, mu_star_hi=0.105):
    ...
    if mu >= mu_star_hi:
        verdict = "互斥超临界"
        notes = [f"S7(μ): μ={mu:.3f}>={mu_star_hi} → 概念瘫痪风险"]
    elif mu >= mu_star_lo:
        verdict = "临界"
        notes = [f"S7(μ): μ={mu:.3f}∈[{mu_star_lo},{mu_star_hi}] → 临界"]
    else:
        verdict = "低互斥/有效"
        notes = [f"S7(μ): μ={mu:.3f}<{mu_star_lo} → 概念健康"]
    return verdict, notes

def reaudit(name, card, mode):
    print(f"\n===== {name}（{mode}，GLM5.2）=====", flush=True)
    ...
    if structs is None or len(structs) != len(variants):
        print(f"  提取数量不匹配({len(structs) if structs else 0}/{len(variants)})，跳过", flush=True)
        return None
    ...
    print(f"  N={N} A_rate(变体/第一律)={A_v:.3f} 评论级={card2['A_rate']:.3f} "
          f"r={r:.3f}[{card2['r_confidence']}] Δr={card2['delta_r']:+.3f} align={align} "
...
def main():
    ...
    print("\n已存 mutex_audit_v6_glm52.json（GLM5.2 对照，未覆盖 v5 系列）", flush=True)
    ...
    for name, c in out.items():
        ...
        if same:
            stable += 1
        print(f"  {name}: v5.6[{old_v}] → v6[{new_v}] {'✓' if same else '✗ 变化'}"
              f" | A变 {old.get('A_rate_variant_law1','?')} → {c.get('A_rate_variant_law1')}"
              f" | align {old.get('align','?')} → {c.get('align')}", flush=True)
